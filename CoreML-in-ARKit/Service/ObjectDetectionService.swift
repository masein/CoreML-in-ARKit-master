...
class ObjectDetectionService {
    var mlModel = try! VNCoreMLModel(for: MugAndBottleObject().model)
//    let mlModel : MLModel = {
//        do {
//            let configuration = MLModelConfiguration()
//            return try YOLOv3Int8LUT(configuration: configuration).model
//        } catch let error {
...
private extension ObjectDetectionService {
    func coreMlRequestHandler(_ request: VNRequest?, error: Error?) {
        ...
        guard let result = results.first(where: { $0.confidence > 0.6 }),
